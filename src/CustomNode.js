...
// import { ArrowHead } from 'react-flow-renderer/dist/types';

const CustomNode = ({ data }) => {
    const [isSourceConnected, setSourceConnected] = useState(false);
    ...
    return (
        <div className='custom-node'>
            <div className='send-message'>
                <div>
                    <b> <MdOutlineMessage size={10} style={{ verticalAlign: "middle" }} /> Send Message</b>
                </div>
            ...
            {/* <ArrowHead type="target" color="#ddd" /> */}
            {/* SOurce node dot on the right-hand side */}
            <Handle
                type="source"
                position="right"
                id="source-node"
                style={{
                    background: isSourceConnected ? '#ccc' : '#555',
                    width: '5px',
                    height: '5px',
                    borderRadius: '50%',
                    cursor: isSourceConnected ? 'not-allowed' : 'pointer'
                }}
                onConnect={handleConnect}
                isConnectable={!isSourceConnected} // Disable connection if the source is already connected
            />

            {/* Target node dot on the left-hand side */}
            <Handle
                type="target"
                position="left"
                id="target-node"
                style={{ background: '#555', width: '5px', height: '5px', borderRadius: '50%' }}
            />
        </div>
    );
};
